...
void	camera_frame_counting(t_data *data)
{
	...
	data->draw_end = screenHeight / 2 + data->line_height / 2  + 100;
	data->draw_start = screenHeight / 2 + 100 - (data->line_height / 2);
...
{
	data->camera_x = 2 * i / (data->widht - 1);
...
void	ft_filling(t_data *data)
{
	...
	data->pitch = 100;
	data->line_height = (int)(screenHeight / data->perp_wall_dist);
	data->draw_start = ((-1 * data->line_height) / 2) + screenHeight / 2 + data->pitch;
